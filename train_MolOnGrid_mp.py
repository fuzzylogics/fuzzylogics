...
from data import MolOnGrid, collate_fn_MolOnGrid, reg_1deriv
import torch
import numpy as np
import pickle
import os
...
def training_process(rank, world_size, dataset, tr_opts, batch_size=3):
    # setting up
    ...
    loader = DataLoader(dataset, 
                        ...
                        sampler=sampler, collate_fn=collate_fn_MolOnGrid)    
    ...
    for itr in range(ini_itr, ini_itr+max_itr):
        sampler.set_epoch(itr)  # sampler reset seed
        model.train()
            ...
            for iks in range(max_iks):    
                ...
                for i_struc_in_batch, mol in enumerate(mol_batch):
                    ...
                    if 'curve_1deriv' in tr_opts['regulators']:
                        curve_reg_1deriv = tr_opts['regulators']['curve_1deriv']['strength']*reg_1deriv(y, mol.grid, device)
                    ...
                    if itr % itr_save == 0 and iks % iks_save == 0:
                        ...
                        with open('./results/rho_diff%d.pkl'%(i_struc), 'ab') as f_rho:
                            print('Saving rho at itr=%d,iks=%d...'%(itr,iks), end='')
                            pickle.dump(rho_diff.cpu().numpy(), f_rho)
                        with open('./results/vxc%d.pkl'%(i_struc), 'ab') as f_v:
...
def main():
    ...
    batch_size = 3
    tr_opts = opts['train']
    dataset = MolOnGrid(opts['molecules'], opts['sampling'], path='./', device='cuda')
    dataset.gen_all()
    ...
    mp.spawn(training_process,
        args=(world_size, dataset, tr_opts, batch_size),
        nprocs=world_size,
        join=True)
