...
from data import MolOnGrid, aprox_Hartree, collate_fn_MolOnGrid, collate_fn_MolOnGrid_w_grid_edges, eval_I, loss_w_grid_weights, reg_1deriv, reg_1deriv_pow, mse, reg_2deriv
import torch
import numpy as np
import pickle
import os
            ...
            for iks in range(max_iks):    
                ...
                for i_struc_in_batch, mol in enumerate(mol_batch):
                    ...
                    if 'wy' in tr_opts['loss_guides']: # WY
                        target = target + tr_opts['loss_guides']['wy'] * rho_diff
